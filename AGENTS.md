...
- In-app navigation (`routes/charger/[id]/navigate`) is last-mile turn-by-turn over the same router. Pure maneuver/snap logic lives in `routing/navigation.ts` (unit-tested in `navigation.test.ts`); the page adds heading-up map rotation (CSS), `SpeechSynthesis` voice, Wake Lock, and off-route reroute. Map orientation is a user-cycled control — North-up / Course-up (GPS movement bearing) / Compass — where Compass uses `lib/sensors/compass.ts` (iOS `webkitCompassHeading` + tap-to-grant permission, Android `deviceorientationabsolute`) and is only offered when supported, falling back to North if denied. Compass math is unit-tested. It is a screen-on preview, not a background nav replacement — keep the Google/Apple/Waze deep links as the real-nav path.
